...
public class DBHelper {
	...
	private Map<String, Integer> tableStartPos = new HashMap<String, Integer>();
	private Map<String, Integer> tableEndPos = new HashMap<String, Integer>();

	// To hold one collumn related Primary Keys Collumn Possition
	// Map<column, result set keys>
	private Map<Integer, List<Integer>> pkPerCollumn;
	public Map<Integer, List<Integer>> getPkPerCollumn() {
		return this.pkPerCollumn;
	}
	...
	// Get The Table Name and It's possition in the ResultSet
	 ...
	 * (pkPerCollumn) which mapping each column with its subsequent primary key
	 * in the result set.
	 */
	private void resultSetPKPos() throws SQLException {
		ResultSet tablePrimaryKeys = null;
		String currentTableName, key;
		this.pkPerCollumn = new HashMap<Integer, List<Integer>>();
		List<Integer> keys;
			...
			while (tablePrimaryKeys.next()) {
				...
				for (int j = tableStartPos.get(tableName); j <= tableEndPos
					...
					if (key.equalsIgnoreCase(currentTableName)) {
						if (this.pkPerCollumn.containsKey(i)) {
							keys = this.pkPerCollumn.get(i);
							keys.add(j);
							this.pkPerCollumn.put(i, keys);
						} else {
							this.pkPerCollumn.put(i, new LinkedList<Integer>());
							keys = this.pkPerCollumn.get(i);
							keys.add(j);
							this.pkPerCollumn.put(i, keys);
						}
	...
	*/
	private void resultSetFKPos() throws SQLException {
		...
		this.foreignKeyColumn = new HashSet<Integer>();
	 ...
	 */
	public Object getObjectFromResultSet(int collumn) throws SQLException {
		Object o = null;
		switch (rsmd.getColumnType(collumn)) {

		case java.sql.Types.BIGINT:
			o = rs.getLong(collumn);
			break;
		...
		case java.sql.Types.BIT:
			o = rs.getBoolean(collumn);
			break;

		case java.sql.Types.REAL:
			o = rs.getFloat(collumn);
			break;

		case java.sql.Types.DECIMAL:
		case java.sql.Types.NUMERIC:
			o = rs.getBigDecimal(collumn);
			break;

		case java.sql.Types.DOUBLE:
		case java.sql.Types.FLOAT:
			o = rs.getDouble(collumn);
			break;
		...
		case java.sql.Types.NVARCHAR:
			...
			o = rs.getString(collumn);
			break;

		case java.sql.Types.BINARY:
		case java.sql.Types.VARBINARY:
		case java.sql.Types.LONGVARBINARY:
			// we're being general here. According to Oracle recomendation, it
			// should be returned as byte[]
			o = rs.getString(collumn);
			break;

		case java.sql.Types.DATE:
			o = rs.getDate(collumn);
			break;

		case java.sql.Types.TIMESTAMP:
			o = rs.getTimestamp(collumn);
			break;

		case java.sql.Types.TIME:
			o = rs.getTime(collumn);
			break;
		...
		case java.sql.Types.JAVA_OBJECT:
			// theese data types are custom, unique, and rarely used
